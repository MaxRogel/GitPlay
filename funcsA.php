...
function GetLocusInfo($nloc){
	...
	//$sql = 'SELECT COUNT(*) FROM pt WHERE Locus_ID = ' . $nloc;	
	//$conn->query($sql)) 
	...
	if ($arCnt > 0) {
		$stmt = $conn -> prepare("SELECT `AR_ID`, `Locus_ID`, `AR_no`, `Related_PT_no`, `Category`, `Date`, `Level`, `Description`, `Notes` FROM ar WHERE Locus_ID = :loc ORDER BY AR_no");
		$stmt->execute([':loc' => $nloc]);
